...
const CourseAspects=()=>{
    ...
    const addRow = () => {
        if (newLine.trim() !== '') {
            axios.post('http://localhost:9000/addCourseAsp',{question:newLine})
            .then(result=>{
  ...
  const deleteRow = (index) => {
    axios.delete('http://localhost:9000/deleteCourseAsp/'+index).then(res=>console.log(res)).catch(err=>console.log(err))
    fetchData();
  };
    ...
    const fetchData = async () => {
        try {
        const response = await axios.get('http://localhost:9000/getCourseAsp');
    ...
    return(
        ...
        <NavComponent/>
        {
            isAdmin ? (
                ...
                <h2 class="abel-600 m-1" style={{fontSize: '1.5rem',color: '#213555'}}>Course Feedback Aspects</h2>
